...
#include <poll.h>
#define NALLOC 10
static struct pollfd *grow_pollfd(struct pollfd *pfd)
{
	int i;
	int oldmax=*maxfd;
...
void loop(void)
{
	...
	for(;;)
	{
		...
		for(i=1;i<numfd;i++)
		{
			...
			{
				log_msg("closed: uid %d,fd %d",client[i].uid,pollfd.fd);
			...
			else handle_request(buf,nread,pollfd[i].fd,client[i].uid);
		}
	}
}
